...
**Prerequisites.** Conditional expectations, covariance matrices, matrix algebra, statistical tests, and large-sample approximations.
...
The coefficients $\beta_k$ summarize how the conditional mean of $y_i$ varies with the regressors under the assumptions introduced below. We estimate them from a sample $\{y_i,\bv{x}_{i}\}_{i=1}^n$, where $n$ denotes the sample size.
...
The columns of the sample design matrix $\bv{X}$ are linearly independent: $\operatorname{rank}(\bv{X})=K$. In particular, this requires $n\ge K$.
:::
...
ii. The $x_{ij}$'s and the $\varepsilon_{i}$'s have zero covariance, i.e. $\forall i,\,j \quad \mathbb{C}ov(x_{ij},\varepsilon_{i})=0$. Their correlation is therefore zero whenever both variances are finite and non-zero.
:::
::: {.proof}
...
```{r heteroskedasticity, fig.cap="Homoskedasticity vs heteroskedasticity. See text for the exact specifications.", fig.asp = .6, out.width = "90%", fig.align = 'left'}
N <- 200
...
Figure \@ref(fig:exmpSalarayPhDSHP) shows a real-data pattern that suggests heteroskedasticity, based on data taken from the [Swiss Household Panel](https://forscenter.ch/projects/swiss-household-panel/). The sample is restricted to persons (i) who are younger than 35 years in 2019, and (ii) who have completed at least 19 years of study. The dispersion of yearly income appears to increase with age, although the graph alone is not a formal test.

```{r exmpSalarayPhDSHP, fig.cap="Income versus age. Data are from the Swiss Household Panel. The sample is restricted to persons that have completed at least 19 years of study. The figure shows that the dispersion of yearly income increases with age.", fig.asp = .6, out.width = "95%", fig.align = "left"}
library(AEC)
...
```
<!-- x -->
<!-- ```{r exmpSalarayPhD, fig.cap="Salary versus years after PhD", fig.asp = .6, out.width = "90%", fig.align = 'left'} -->
<!-- # load data into R -->
...
$$
\boldsymbol\varepsilon\mid\bv{X}\sim\mathcal{N}(\bv{0},\sigma^2Id_n).
$$
:::


## Least-squares estimation {#LSquares}

### Derivation of the OLS formula

With the assumptions and notation in place, we now turn to estimation. **Ordinary Least Squares (OLS)** chooses the coefficient vector $\bv{b}$ that minimizes the sum of squared residuals. Residuals are the sample counterparts of the unobserved errors $\varepsilon_i$.
...
Since:^[see Proposition \@ref(prp:partial).]
$$
...
\end{equation}
where $\bv{M} := Id - \bv{X} (\bv{X}'\bv{X})^{-1} \bv{X}'$ is called the **residual maker** matrix. 
...
$$
\bv{b} = (\bv{X}'\bv{X})^{-1} \bv{X}'\bv{y} = \boldsymbol\beta + (\bv{X}'\bv{X})^{-1} \bv{X}' {\boldsymbol\varepsilon}.
$$

i. Let us consider the expectation of the last term, i.e. $\mathbb{E}((\bv{X}'\bv{X})^{-1} \bv{X}' {\boldsymbol\varepsilon})$. Using the law of iterated expectations, we obtain:
$$
\mathbb{E}((\bv{X}'\bv{X})^{-1} \bv{X}' {\boldsymbol\varepsilon}) = \mathbb{E}(\mathbb{E}[(\bv{X}'\bv{X})^{-1} \bv{X}' {\boldsymbol\varepsilon}|\bv{X}]) = \mathbb{E}((\bv{X}'\bv{X})^{-1} \bv{X}'\mathbb{E}[{\boldsymbol\varepsilon}|\bv{X}]).
$$
By Hypothesis \@ref(hyp:exogeneity), we have $\mathbb{E}[{\boldsymbol\varepsilon}|\bv{X}]=0$. Hence $\mathbb{E}((\bv{X}'\bv{X})^{-1} \bv{X}' {\boldsymbol\varepsilon}) =0$ and result (i) follows.
...
* $\bv{M}^{\bv{X}}$: residual-maker matrix of any regression on $\bv{X}$ (given by $Id - \bv{X} (\bv{X}'\bv{X})^{-1} \bv{X}'$),
...
\begin{eqnarray*}
\bv{b}_2 &=& [\bv{X}_2'\bv{X}_2 - \bv{X}_2'\bv{X}_1(\bv{X}_1'\bv{X}_1)^{-1}\bv{X}_1'\bv{X}_2]^{-1}\bv{X}_2'(Id - \bv{X}_1(\bv{X}_1'\bv{X}_1)^{-1}\bv{X}_1')\bv{y}\\
...
\end{equation}
Now $\bv{z^*}'\bv{e} = \bv{z^*}'(\bv{y} - \bv{X}\bv{b}) = \bv{z^*}'\bv{y}$ because $\bv{z}^*$ are the residuals in an OLS regression on $\bv{X}$. Since $c = (\bv{z^*}'\bv{z^*})^{-1}\bv{z^*}'\bv{y^*}$ (by an application of Theorem \@ref(thm:FW)), we have $(\bv{z^*}'\bv{z^*})c = \bv{z^*}'\bv{y^*}$ and, therefore, $\bv{z^*}'\bv{e} = (\bv{z^*}'\bv{z^*})c$. Substituting this expression into Eq. \@ref(eq:uuu) proves the result.
:::
...
:::
:::{.proof}
Using the notation of Proposition \@ref(prp:chgeR2), Theorem \@ref(thm:FW) implies that $c = (\bv{z^*}'\bv{z^*})^{-1}\bv{z^*}'\bv{y^*}$. Substitution into Eq. \@ref(eq:uu) gives $\bv{u}'\bv{u} = \bv{e}'\bv{e} - (\bv{z^*}'\bv{y^*})^2/(\bv{z^*}'\bv{z^*})$. By the definition of partial correlation in Eq. \@ref(eq:pc), $\bv{u}'\bv{u} = \bv{e}'\bv{e}\left(1 - (r_{yz}^\bv{X})^2\right)$. Dividing both sides by $\bv{y}'\bv{M}_0\bv{y}$ proves the result.
:::
...
```{r R2issue, echo=TRUE, fig.cap="This figure illustrates the monotonous increase in the $R^2$ as a function of the number of explanatory variables. In the true model, there is no explanatory variables, i.e., $y_i = \\varepsilon_i$. We then take (independent) regressors and regress $y$ on the latter, progressively increasing the set of regressors.", fig.asp = .6, out.width = "90%", fig.align = 'left'}
...
Good fit does not by itself imply precise coefficient estimates. Inference requires the sampling distribution of $\bv{b}$, which is available exactly under the Gaussian assumptions stated above.
...
data_table(unclass(lmtest::coeftest(eq)), digits=3, row_label="Term")
```
:::
...
data_table(cbind("Lower 95%"=lower.b, "Upper 95%"=upper.b),
           digits=3, row_label="Term")
```


### Testing a set of linear restrictions {#Ftest}

A t-test concerns one coefficient or one linear combination. Many economic hypotheses impose several restrictions at once, so we next test whether $J$ linear restrictions are *jointly* consistent with the data:
...
Under Assumptions \@ref(hyp:fullrank) to \@ref(hyp:noncorrelResid), suppose that the $(\bv{x}_i,\varepsilon_i)$ are i.i.d. with the finite moments required below and that:
\begin{equation}
...
\begin{eqnarray*}
(\bv{X}'\bv{X})^{-1} &=& \frac{1}{1 - correl_{1,2}^2}\left[ \begin{array}{cc}
...
Hence, the closer to one $correl_{1,2}$, the higher the variance of $b_1$ (recall that the variance of $b_1$ is the upper-left component of $\sigma^2(\bv{X}'\bv{X})^{-1}$). That is, if some of our regressors are close to a linear combination of the other ones, then the confidence intervals will tend to be wide, which typically reduces the power of the t-test (we tend to fail to reject the null hypothesis that the coefficients are different from zero).
...
model_table(eq1,eq2,eq3,no.space = TRUE,omit.stat=c("f","ser"))
```
:::
...
Omitted variables are one reason why a regressor may be correlated with the error. Simultaneity and measurement error can create the same problem. When the conditional mean-zero assumption $\mathbb{E}(\boldsymbol\varepsilon|\bv{X})=0$ is implausible, OLS generally fails to estimate the parameter consistently. Instrumental variables recover identifying variation from variables that are related to the endogenous regressors but orthogonal to the structural error.
...
This two-step approach is called **Two-Stage Least Squares (2SLS)**. It results in:
\begin{equation}
...
model_table(eq.no.IV,eq.IV1,eq.IV2,no.space = TRUE,
                     omit.stat=c("f","ser"))
data_table(summary(eq.IV1,diagnostics = TRUE)$diagnostics,
           digits=3, row_label="Diagnostic")
```
...
We have $\boldsymbol\Sigma = (\bv{P}\bv{P}')^{-1}$ with $\bv{P} = \bv{C}\boldsymbol\Lambda^{-1/2}$. (We also have $\bv{P}=\boldsymbol\Sigma^{-1/2}$.) Consider the transformed model:
$$
...
<!-- the OLS is not efficient. However, it is asymptotically consistent and normal (Props. \@ref(prp:ols-grm-consistency) and \@ref(prp:AsymptGRM), respectively). -->
...
:::{.proposition #ols-grm-consistency name="Consistency of the OLS estimator in the GRM framework"}
...
```{r simulHeterosk, echo=TRUE, fig.cap="Situation of heteroskedasticity. The model is $y_i = x_i + \\varepsilon_i, \\quad \\varepsilon_i \\sim \\mathcal{N}(0,x_i^2)$, where the $x_i$'s are i.i.d. $t(6)$.", fig.asp = .6, out.width = "90%", fig.align = "left"}
n <- 200
x <- rt(n,df=6)
y <- x + x*rnorm(n)
par(plt=c(.1,.95,.1,.95))
...
c(sd(all_b),mean(sqrt(all_V_OLS)),
  mean(sqrt(all_V_White_HC0)),mean(sqrt(all_V_White_HC1)))
```
...
\begin{equation}
y_i = x_i + \varepsilon_i,(\#eq:simul11)
\end{equation}
...
data_table(data.frame("Simulation SD"=sd(all.b),
  ...
  "Mean HAC SE"=mean(all.NW.stdv.b), check.names=FALSE), digits=3)
```
...
These results show why autocorrelation-robust inference matters. Before choosing such an adjustment, however, we may want to diagnose serial correlation in the residuals. The Durbin-Watson test, proposed by @Durbin_Watson_1950 and @Durbin_Watson_1951, uses the statistic
$$
...
<!-- |                                                      | Under Assumptions \@ref(hyp:fullrank)+ | $\bv{b}$ normal in small sample (Eq. \@ref(eq:distriBcondi)) | $\bv{b}$ is BLUE (Thm \@ref(thm:GaussMarkov)) | $\bv{b}$ unbiased in small sample (Prop. \@ref(prp:propOLS)) | $\bv{b}$ consistent (Prop. \@ref(prp:ols-grm-consistency))$^*$ | $\bv{b}$ $\sim$ normal in large sample (Prop. \@ref(prp:AsymptGRM))$^*$ | -->
<!-- |------------------------------------------------------|-----------------------------------------|-------------------------------------------------------------|----------------------------------------------|--------------------------------------------------------------|------------------------------------------------|--------------------------------------------------------------------------| -->
...
<!-- $^*$: see however Prop. \@ref(prp:ols-grm-consistency) and Prop. \@ref(prp:AsymptGRM) for additional hypotheses. Specifically $\bv{X}'\bv{X}/n$ and $\bv{X}'\boldsymbol{\Sigma}\bv{X}/n$ must converge in proba. to finite positive definite matrices ($\boldsymbol\Sigma$ is defined in Eq. \@ref(eq:assumGLS2)). -->



### Cluster-robust covariance matrices {#Clusters}

HAC estimators exploit an ordering of observations. In many cross-sectional and panel applications, dependence is instead organized by groups. Cluster-robust inference allows arbitrary dependence within a cluster while assuming independence across clusters. A cluster may gather entities from the same geographical area, industry, or cohort. This section follows @MACKINNON2022; another useful reference is @Cameron_Miller_2014.
...
As an alternative to an asymptotic approximation, one can use the bootstrap methods discussed in Section 5 of @MACKINNON2022. In R, the package `fwildclusterboot` implements such procedures.[^FootWildCluster]
...
x <- subset(credit_lasso,
            select = c(delinq_2yrs,annual_inc,annual_inc2,log_annual_inc,
                       ...
                       owner,mortgage,pub_rec_bankruptcies,revol_util,revol_bal))
```

Let us standardize the data:

```{r lasso2}
y <- scale(credit_lasso$int_rate)
x <- scale(x)
indic_complete_cases <- complete.cases(x)
y <- y[indic_complete_cases]  # to remove observations with NaN's
x <- x[indic_complete_cases,] # to remove observations with NaN's
```
...
```{r lasso5}
i <- 20; j <- 40
data_table(cbind("Lambda i"=result.lasso$lambda[i],
                 "Lambda j"=result.lasso$lambda[j]), digits=3)
data_table(cbind("Lambda i"=coef(result.lasso)[,i],
                 "Lambda j"=coef(result.lasso)[,j]), digits=4, row_label="Term")
...
```{r regressions-recap-space, echo=FALSE, results='asis'}
if (knitr::is_latex_output()) cat("\\Needspace{16\\baselineskip}\n")
```

## Chapter recap {#regressions-recap}
...
```{r regressions-exercise-pagebreak, echo=FALSE, results='asis'}
if (knitr::is_latex_output()) cat("\\clearpage\n")
```

## Exercises {#regressions-exercises}
...
```{r regressions-exercise-bank, echo=FALSE, results='asis'}
source("R/exercises.R")
render_exercises("regressions")
```

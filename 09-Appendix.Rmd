...
**Prerequisites.** The notation introduced in the chapter that refers to the relevant appendix result.

**After consulting the relevant sections, you should be able to:**
...
**Roadmap.** The appendix moves from principal components and linear algebra to statistical definitions and distributions, stochastic convergence, Gaussian results, selected proofs, and statistical tables.



## Principal component analysis (PCA) {#PCAapp}

**Principal component analysis (PCA)** reduces the dimension of a dataset when much of its variation can be summarized by a small number of linear combinations. The method is widely used in economic data analysis, visualization, and image compression.
...
The term structure of interest rates provides a standard application. Yields at different maturities move together and can often be summarized by a few factors [see @Litterman_Scheinkman_1991]. We apply PCA to data from the [FRED database](https://fred.stlouisfed.org) (series `DGS6MO`, `DGS1`, and related maturities). The estimated loadings can be interpreted as level, slope, and curvature factors.
...
```{r USydsPCA1, warning=FALSE,message=FALSE, fig.cap="Some PCA results. The dataset contains 8 time series of U.S. interest rates of different maturities.", fig.align = 'left'}
par(mfrow=c(2,2))
par(plt=c(.1,.95,.2,.8))
...
:::{.definition #eigenvalues name="Eigenvalues"}
...
:::{.proposition #MoorPenrose-properties name="Properties of the Moore-Penrose inverse"}
...
Consider a function $f:\mathbb{R}^K\rightarrow\mathbb{R}$. Its first derivative is
$$
...
* If $f(\bv{b}) = \bv{b}'A\bv{b}$ where $A$ is a $K \times K$ matrix, then $\frac{\partial f}{\partial \bv{b}}(\bv{b}) = (A+A')\bv{b}$. If $A$ is symmetric, this simplifies to $2A\bv{b}$.
:::
...
Assuming $X$ and $Y$ have finite variances, we have:
$$
...
:::{.definition #asmyptlevelAppendix name="Asymptotic level"}
...
:::{.definition #asmyptconsisttestAppendix name="Asymptotically consistent test"}
...
:::{.proposition #Kullback-properties name="Properties of the Kullback discrepancy"}
We have:
...
```{r Cauchy, fig.cap="Pdf of the Cauchy distribution ($\\mu=0$, $\\gamma=1$).", fig.asp = .6, out.width = "95%", fig.align = "left",echo=FALSE,warning=FALSE}
x <- seq(-5,5,by=.1)
...
The random variable sequence $x_n$ converges in probability to a constant $c$ if, for every $\varepsilon>0$, $\lim_{n \rightarrow \infty} \mathbb{P}(|x_n - c|>\varepsilon) = 0$.
...
:::{.theorem #LindbergLevyCLTAppendix name="Lindberg-Levy Central limit theorem, CLT"}
...
:::
:::{.proof}
If $\bv{L}\bv{A}=\bv{0}$, then the Gaussian vectors $\bv{L}\bv{x}$ and $\bv{A}\bv{x}$ are independent, as are any functions of those vectors. The result follows because $\bv{x}'\bv{A}\bv{x}=(\bv{A}\bv{x})'(\bv{A}\bv{x})$ is a function of $\bv{A}\bv{x}$.
:::
...
```{r inverseMills, echo=FALSE, fig.cap="$\\mathbb{E}(X|X<b)$ as a function of $b$ when $X\\sim \\mathcal{N}(0,1)$ (in black).", fig.align = 'left'}
x <- seq(-10,10,by=.01)
par(mfrow=c(1,1))
...
If $Y \sim \mathcal{N}(\mu,\Omega)$, $Y$ is an $n$-dimensional vector, and $\Omega$ is positive definite, then the density function of $Y$ is:
$$
...
:::{.proof}
We have $\sqrt{n}(\hat{\boldsymbol\theta}_{n} - \boldsymbol\theta_{0}) \overset{d}{\rightarrow} \mathcal{N}(0,\mathcal{I}(\boldsymbol\theta_0)^{-1})$ (Eq. \@ref(eq:normMLE)). A Taylor expansion around $\boldsymbol\theta_0$ gives
\begin{equation}
\sqrt{n}(h(\hat{\boldsymbol\theta}_{n}) - h(\boldsymbol\theta_{0})) \overset{d}{\rightarrow} \mathcal{N}\left(0,\frac{\partial h(\boldsymbol\theta_{0})}{\partial \boldsymbol\theta'}\mathcal{I}(\boldsymbol\theta_0)^{-1}\frac{\partial h(\boldsymbol\theta_{0})'}{\partial \boldsymbol\theta}\right). (\#eq:wald-delta-limit)
\end{equation}
...
\mathcal{I}(\boldsymbol\theta_0)^{-1}\frac{1}{\sqrt{n}}\frac{\partial \log \mathcal{L}(\hat{\boldsymbol\theta}^0_n;\bv{y})}{\partial \boldsymbol\theta}.(\#eq:lm2)
\end{equation}
...
\begin{equation}
\xi_n^{LR} \approx n (\hat{\boldsymbol\theta}^0_n-\hat{\boldsymbol\theta}_n)' \mathcal{I}(\boldsymbol\theta_0) (\hat{\boldsymbol\theta}^0_n-\hat{\boldsymbol\theta}_n). (\#eq:lr10)
\end{equation}
...
**Gaussian multivariate regression: likelihood derivation**
...
colnames(table_N01) <- sprintf("%.2f", columns)
rownames(table_N01) <- sprintf("%.1f", rows)
data_table(table_N01, long=TRUE, row_label="a", caption = "Cumulative probabilities of the $\\mathcal{N}(0,1)$ distribution. If $a$ and $b$ are respectively the row and column number; then the corresponding cell gives $\\mathbb{P}(X\\le a+b)$, where $X \\sim \\mathcal{N}(0,1)$.", digits=4)
```
...
data_table(table_Student, long=TRUE, row_label="df", caption = "Quantiles of the Student-$t$ distribution. The rows correspond to different degrees of freedom ($\\nu$, say); the columns correspond to different probabilities ($z$, say). The cell gives $q$ that is s.t. $\\mathbb{P}(-q<X<q)=z$, with $X \\sim t(\\nu)$.", digits=3)
```
...
data_table(table_chi2, long=TRUE, row_label="df", caption = "Quantiles of the $\\chi^2$ distribution. The rows correspond to different degrees of freedom; the columns correspond to different probabilities.", digits=3)
```
